...
# ---------------------------------------------------------------------------
# Helpers: key generation & encryption
# ---------------------------------------------------------------------------

def random_key(n: int, seed: int = 42) -> np.ndarray:
    """Generate a random invertible NxN key matrix."""
    rng = np.random.RandomState(seed)
    while True:
        K = rng.randn(n, n).astype(np.float32)
        if abs(np.linalg.det(K)) > 0.1:  # ensure invertible
            return K
...
class TestTransformEncryption:
    ...
    def test_large_matrix_50x50(self):
        ...
        np.testing.assert_allclose(decrypt_vec(result_enc, K), result, atol=1e-2)
...
class TestDistanceEncryption:
    ...
    @staticmethod
    def random_orthogonal_key(n: int, seed: int = 42) -> np.ndarray:
    ...
    def test_euclidean_preserved(self):
        K = self.random_orthogonal_key(4)
    ...
    def test_cosine_preserved(self):
        K = self.random_orthogonal_key(3)
    ...
    def test_dot_product_preserved(self):
        K = self.random_orthogonal_key(5)
